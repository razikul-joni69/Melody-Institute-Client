...
const Sidebar = () => {
    ...
    return (<div>
        <button onClick={toggleSidebar}>{!sidebarVisible &&
            <AiOutlineMenuUnfold className={`h-6 w-6 text-green-600`}/>} </button>

        <div className="block md:flex">
            <div id="sidebar"
                 className={`${sidebarVisible ? 'md:visible' : 'hidden'} w-full h-full p-3 space-y-2 md:w-60 bg-gray-50 text-gray-800`}>
                <div className="flex items-center justify-between">
                    <h2 className="text-lg">Dashboard</h2>
                    <button onClick={toggleSidebar}>{sidebarVisible ? <CgClose className={`h-6 w-6 text-red-600`}/> :
                        <AiOutlineMenuUnfold className={`h-6 w-6`}/>} </button>
                </div>
			...
			<span className="absolute inset-y-0 left-0 flex items-center py-4">
                ...
                <div className="grid grid-cols-1 justify-items-center">
                    <img src="https://source.unsplash.com/100x100/?portrait" alt=""
                         className="w-26 h-26 rounded-full bg-gray-500"/>
                    <div>
                        <h2 className="text-lg font-semibold">Leroy Jenkins sdfasdf </h2>
                        <h5 className="text-center"><span
                            className="badge badge-info text-white">user</span></h5>
                    </div>
                </div>
                <div className="divide-y divide-gray-300">
                    <ul className="pt-2 pb-4 space-y-1 text-lg">
                        ...
                        <li>
                            <NavLink to="/dashboard/" rel="noopener noreferrer" href="#"
                                     className="flex items-center p-2 space-x-3 rounded-md">
                                <RiSecurePaymentLine/>
